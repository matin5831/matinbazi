      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        ...
        <div className="text-center mb-10 animate-fade-in">
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 mb-3">
            بازی و جایزه <span className="text-amber-600">🎁</span>
          </h1>
          <p className="text-slate-500 text-sm leading-relaxed max-w-xl mx-auto">
            با <strong className="text-amber-600">۵ بازی شانسی</strong> سرگرم شوید و <strong className="text-amber-600">کد تخفیف</strong> برنده شوید!
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-8">
          {GAME_INFO.map((g, i) => (
            <div
              key={g.title}
              className="bg-slate-50/70 backdrop-blur border border-slate-200 hover:border-amber-300 rounded-2xl p-5 transition-all hover:scale-[1.02] animate-fade-in"
              style={{ animationDelay: `${i * 60}ms` }}
            >
              <div className="flex items-start gap-3.5">
                <span className="text-3xl shrink-0">{g.icon}</span>
                <div>
                  <h3 className="text-sm font-black text-slate-900 mb-1.5">{g.title}</h3>
        ...
        {/* How it works */}
        <div className="bg-slate-50/70 border border-slate-200 rounded-2xl p-5 mb-8">
          <h3 className="text-sm font-black text-amber-600 mb-4 flex items-center gap-2">
            <Sparkles className="w-4 h-4" />
            چطور شرکت کنم؟
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-[11.5px] text-slate-500 leading-relaxed">
            <div className="flex items-start gap-2.5">
              <Instagram className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
              <span>پیج اینستاگرام فروشگاه را <strong className="text-slate-600">فالو کنید</strong></span>
            </div>
            <div className="flex items-start gap-2.5">
              <Phone className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
              <span>آیدی اینستاگرام و <strong className="text-slate-600">شماره همراه</strong> خود را وارد کنید</span>
            </div>
            <div className="flex items-start gap-2.5">
              <Ticket className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
              <span>یکی از ۵ بازی را انتخاب کنید و <strong className="text-slate-600">کد تخفیف</strong> بگیرید!</span>
            </div>
          </div>
        </div>

        {/* Rules */}
        <div className="bg-amber-50 border border-amber-300 rounded-2xl p-4 flex items-start gap-3 mb-10">
          <ShieldCheck className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
          <p className="text-[11px] text-amber-700/90 leading-relaxed">
            <strong className="text-amber-600">قوانین:</strong> هر آیدی اینستاگرام و شماره همراه فقط یک بار مجاز به شرکت در هر کمپین است.
            کد تخفیف به صورت خودکار صادر می‌شود و تا پایان کمپین اعتبار دارد.
          </p>
        </div>

        {/* Footer */}
        <footer className="border-t border-slate-200/80 pt-5 flex items-center justify-center gap-2 text-[11px] text-slate-500">
          <Gift className="w-3.5 h-3.5 text-amber-500" />
          <span>متین بازی — پلتفرم گیمیفیکیشن و بازاریابی تعاملی</span>
          <span className="px-2 py-0.5 rounded-full bg-white border border-slate-200 text-amber-600 font-bold text-[10px]">
            v{APP_VERSION}
          </span>
        </footer>
